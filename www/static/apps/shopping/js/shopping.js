/*
----------------------------------------------
Grope Shopping common JavaScript
...
function cm_showPagerNav(count,pageindex,pagesize,pagecount,action)
{
    ...
    if (pageindex > 1) {
        forward += '--><li class="prev"><a href="javascript:' + action + '(' + (pageindex - 1) + ');">&lt; Prev</a></li><!--';
    }

    if (maxpage > pageindex) {
    	next += '--><li class="next"><a href="javascript:' + action + '(' + (pageindex + 1) + ');">Next &gt;</a></li><!--';
    }
    ...
    //current nav number
    pagercurrent = '--><li class="stay">' + pageindex + '</li><!--';
    ...
    nav = '<ul class="resultNav"><!--' + forward + pagerleft + pagercurrent + pagerright + next + '--></ul>';
    return nav;
}
