...
var valid = new Validation('frmEditCar', {immediate:true,useTitles:true});
...
function doSubmit() 
{
	...
	var frmSubmit = $('frmEditCar');
    frmSubmit.action = UrlConfig.BaseUrl + '/ajaxaparking/editcar';
    frmSubmit.request({
         onCreate : loading,
    ...
    try {    
        if (response.responseText != '' && response.responseText == 'true') {      
	        ...
	        new PeriodicalExecuter(function(pe) {
	           window.location = UrlConfig.BaseUrl + '/aparking/editcar?cid=' + $('cid').value;
	           pe.stop();
	           }, 3);
            ...
            $('loading').hide();
            $('errMsg').update('<p>自動車更新が失敗しました。</p>');           
            $('step1').show();
            $('errMsg').show();         
        }
    } catch (e) {
