...
var valid = new Validation('frmEditBackground', {immediate:true,useTitles:true});
...
function doSubmit() 
{
	...
	var frmSubmit = $('frmEditBackground');
    frmSubmit.action = UrlConfig.BaseUrl + '/ajaxaparking/editbackground';
    frmSubmit.request({
         onCreate : loading,
         onSuccess : renderResults4AddBackground
    });
}
 ...
 * @return void
 */
function renderResults4AddBackground(response)
{ 
    try {    
        if (response.responseText != '' && response.responseText == 'true') {      
	        ...
	        new PeriodicalExecuter(function(pe) {
	           window.location = UrlConfig.BaseUrl + '/aparking/editbackground?id=' + $('id').value;
	           pe.stop();
	           }, 3);
            ...
            $('loading').hide();
            $('errMsg').update('<p>不動産更新が失敗しました。</p>');           
            $('step1').show();
            $('errMsg').show();         
        }
    } catch (e) {
