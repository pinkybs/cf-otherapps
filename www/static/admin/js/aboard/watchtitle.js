/**
 * watchtitle list(static/admin/js/aboard/watchtitle.js)
 * watchtitle list 
 * 
...
{
	$('divMsg').hide();
    $('frmDeal').action = UrlConfig.BaseUrl + '/ajaxaboard/dealwatchtitle';
    $('frmDeal').request({
         onCreate : function() {
         	$('btnSubmit').disable();
         },
...
{   
    ...
    var url = UrlConfig.BaseUrl + '/ajaxaboard/listwatchtitle';
...
function showInfo(array)
{
    ...
    for (var i = 0 ; i < array.length ; i++) {        
        ...
        var selected4 = '';
        var selected5 = '';
        if (1 == array[i].title_status || 2 == array[i].title_status) {
        	selected1 = ' selected="selected" ';
        }
        else if (3 == array[i].title_status) {
        	selected3 = ' selected="selected" ';
        }
        else if (4 == array[i].title_status) {
        	selected4 = ' selected="selected" ';
        }
        else if (5 == array[i].title_status) {
        	selected5 = ' selected="selected" ';
        }
        
        var strDisabled = '';
        var strHidInput = '<input type="hidden" id="titleId' + i + '" name="titleId[]" value="' + array[i].uid + '" />';
        if ($F('hidIsWatcher') && 3 <= array[i].title_status) {
        ...
        var strAdminor = array[i].t_admin_id == null ? '' : '<a href="javascript:void(0);" onclick="showType(3,\'' + array[i].t_admin_id + '\');return false;">' + array[i].admin_name.escapeHTML() + '</a>';
        ...
        if ('2' != $F('hidShowType')) {
       	...
       	var strContent = '';
       	if (null != array[i].title && '' != array[i].title) {
       		strContent = array[i].title.escapeHTML().replace($F('hidSrhKeyword'), '<span style="background:#ffff99;">' + $F('hidSrhKeyword') + '</span>');
       	}
