/**
 * watchdes list(static/admin/js/aboard/watchdes.js)
 * watchdes list 
 * 
 * @copyright  Copyright (c) 2008 Community Factory Inc. (http://communityfactory.com)
 * @create    2009/03/13    zhangxin   
 */
...
{
	$('divMsg').hide();
    $('frmDeal').action = UrlConfig.BaseUrl + '/ajaxaboard/dealwatchdes';
    $('frmDeal').request({
         onCreate : function() {
         	$('btnSubmit').disable();
         },
...
{   
    ...
    var url = UrlConfig.BaseUrl + '/ajaxaboard/listwatchdes';
...
function showInfo(array)
{
    ...
    for (var i = 0 ; i < array.length ; i++) {        
        ...
        var selected4 = '';
        var selected5 = '';
        if (1 == array[i].des_status || 2 == array[i].des_status) {
        	selected1 = ' selected="selected" ';
        }
        else if (3 == array[i].des_status) {
        	selected3 = ' selected="selected" ';
        }
        else if (4 == array[i].des_status) {
        	selected4 = ' selected="selected" ';
        }
        else if (5 == array[i].des_status) {
        	selected5 = ' selected="selected" ';
        }
        
        var strDisabled = '';
        var strHidInput = '<input type="hidden" id="desId' + i + '" name="desId[]" value="' + array[i].uid + '" />';
        if ($F('hidIsWatcher') && 3 <= array[i].des_status) {
        ...
        var strSelect = '<select id="selStatus' + i + '" name="selStatus[]" ' + strDisabled + '>' 
                      ...
                      + strHidInput;
        var strCommentor = '<a href="javascript:void(0);" onclick="showType(1,\'' + array[i].uid + '\');return false;">' + array[i].uid.truncate2(12) + '</a>';
        var strObjector = '<a href="javascript:void(0);" onclick="showType(2,\'' + array[i].uid + '\');return false;">' + array[i].uid.truncate2(12) + '</a>';
        var strAdminor = array[i].d_admin_id == null ? '' : '<a href="javascript:void(0);" onclick="showType(3,\'' + array[i].d_admin_id + '\');return false;">' + array[i].admin_name.escapeHTML() + '</a>';
        ...
        if ('2' != $F('hidShowType')) {
       	...
       	var strContent = '';
       	if (null != array[i].introduce && '' != array[i].introduce) {
       		strContent = array[i].introduce.escapeHTML().replace($F('hidSrhKeyword'), '<span style="background:#ffff99;">' + $F('hidSrhKeyword') + '</span>');
        }
