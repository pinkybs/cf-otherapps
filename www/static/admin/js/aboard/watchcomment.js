/**
 * watchcomment list(static/admin/js/aboard/watchcomment.js)
 * watchcomment list 
 * 
 * @copyright  Copyright (c) 2008 Community Factory Inc. (http://communityfactory.com)
 * @create    2009/03/09    zhangxin   
 */
...
{
	$('divMsg').hide();
    $('frmDeal').action = UrlConfig.BaseUrl + '/ajaxaboard/dealwatchcomment';
    $('frmDeal').request({
         onCreate : function() {
         	$('btnSubmit').disable();
         },
...
{   
    ...
    var url = UrlConfig.BaseUrl + '/ajaxaboard/listwatchcomment';
...
function showInfo(array)
{
    ...
    for (var i = 0 ; i < array.length ; i++) {        
        ...
        var selected4 = '';
        var selected5 = '';
        if (1 == array[i].comment_status || 2 == array[i].comment_status) {
        	selected1 = ' selected="selected" ';
        	selected3 = ' selected="selected" ';
        }
        else if (3 == array[i].comment_status) {
        	selected3 = ' selected="selected" ';
        }
        else if (4 == array[i].comment_status) {
        	selected4 = ' selected="selected" ';
        }
        else if (5 == array[i].comment_status) {
        	selected5 = ' selected="selected" ';
        }
        
        var strDisabled = '';
        var strHidInput = '<input type="hidden" id="commentId' + i + '" name="commentId[]" value="' + array[i].bid + '" />';
        if ($F('hidIsWatcher') && 3 <= array[i].comment_status) {
        ...
        var strSelect = '<select id="selStatus' + i + '" name="selStatus[]" ' + strDisabled + '>' 
                      ...
                      + strHidInput;
        var strCommentor = '<a href="javascript:void(0);" onclick="showType(1,\'' + array[i].comment_uid + '\');return false;">' + array[i].comment_uid.truncate2(12) + '</a>';
        var strObjector = '<a href="javascript:void(0);" onclick="showType(2,\'' + array[i].uid + '\');return false;">' + array[i].uid.truncate2(12) + '</a>';
        var strAdminor = array[i].admin_id == null ? '' : '<a href="javascript:void(0);" onclick="showType(3,\'' + array[i].admin_id + '\');return false;">' + array[i].admin_name.escapeHTML() + '</a>';
                
        html += '<tr class="' + cssClass + '">'
              + '    <td>' + array[i].create_time + '</td>';
        if ('1' != $F('hidShowType')) {
        ...
        if ('2' != $F('hidShowType')) {
       	...
       	//pic
       	if (1 == array[i].type) {
       		strContent = '<img src="' + UrlConfig.PhotoUrl + '/apps/board/flash' + array[i].pic_url + '" />';       		
       	}
       	else {
       	    if (null != array[i].content && '' != array[i].content) {
       	    	strContent = array[i].content.escapeHTML().replace($F('hidSrhKeyword'), '<span style="background:#ffff99;">' + $F('hidSrhKeyword') + '</span>');
       	    }
       	}
