...
{
	$('divMsg').hide();
    $('frmDeal').action = UrlConfig.BaseUrl + '/ajaxaschool/dealwatchtopic';
    $('frmDeal').request({
         onCreate : function() {
         	$('btnSubmit').disable();
         },
...
{   
    ...
    var url = UrlConfig.BaseUrl + '/ajaxaschool/listwatchtopic';
...
function showInfo(array)
{
    ...
    for (var i = 0 ; i < array.length ; i++) {        
        ...
        var strDisabled = '';
        var strHidInput = '<input type="hidden" id="tId' + i + '" name="tId[]" value="' + array[i].tid + '" />';
        ...
        var strObjector = array[i].tid;//'<a href="javascript:void(0);" onclick="showType(2,\'' + array[i].uid + '\');return false;">' + array[i].tid + '</a>';
        ...
        html += '<tr class="' + cssClass + '">'
              + '    <td width="20">' + array[i].format_time + '</td>';
        if ('1' != $F('hidShowType')) {
        	html += '    <td width="10">' + strCommentor + '</td>';
        }
        if ('2' != $F('hidShowType')) {
        	html += '    <td width="10">' + strObjector + '</td>';
       	}
       	...
       	if (null != array[i].introduce && '' != array[i].introduce) {
       		strInfo = array[i].introduce.escapeHTML().replace($F('hidSrhKeyword'), '<span style="background:#ffff99;">' + $F('hidSrhKeyword') + '</span>');
       	}
        html += '    <td width="100">' + strContent + '</td>'
        	  + '    <td width="100">' + strInfo + '</td>'
              + '    <td width="120">' + strSelect + '</td>';
        if ('3' != $F('hidShowType')) {
