...
{
	$('divMsg').hide();
    $('frmDeal').action = UrlConfig.BaseUrl + '/ajaxaschool/dealwatchenquiry';
    $('frmDeal').request({
         onCreate : function() {
         	$('btnSubmit').disable();
         },
...
{   
    ...
    var url = UrlConfig.BaseUrl + '/ajaxaschool/listwatchenquiry';
...
function showInfo(array)
{
    ...
    for (var i = 0 ; i < array.length ; i++) {        
        ...
        var strDisabled = '';
        var strHidInput = '<input type="hidden" id="qId' + i + '" name="qId[]" value="' + array[i].qid + '" />';
        ...
        var strObjector = array[i].qid;//'<a href="javascript:void(0);" onclick="showType(2,\'' + array[i].uid + '\');return false;">' + array[i].qid + '</a>';
        ...
        if ('2' != $F('hidShowType')) {
       	...
       	var strContent = '';
       	if (null != array[i].question && '' != array[i].question) {
       		strContent = array[i].question.escapeHTML().replace($F('hidSrhKeyword'), '<span style="background:#ffff99;">' + $F('hidSrhKeyword') + '</span>');
       	}
